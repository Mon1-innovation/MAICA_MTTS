...
# 一定要设置的部分
parser.add_argument('-m', '--model_path', type=str, default=f"logs/44k/G_20000.pth", help='模型路径')
parser.add_argument('-c', '--config_path', type=str, default=f"logs/44k/G_20000.json", help='配置文件路径')
...
parser.add_argument('-cm', '--cluster_model_path', type=str, default=f"pretrain/hubert-soft-0d54a1f4.pt/false", help='聚类模型或特征检索索引路径，留空则自动设为各方案模型的默认路径，如果没有训练聚类或特征检索则随便填')
...
parser.add_argument('-f0p', '--f0_predictor', type=str, default="rmvpe", help='选择F0预测器,可选择crepe,pm,dio,harvest,rmvpe,fcpe默认为pm(注意：crepe为原F0使用均值滤波器)')
...
# 浅扩散设置
parser.add_argument('-dm', '--diffusion_model_path', type=str, default=f"logs/44k/diffusion/model_0.pt/false", help='扩散模型路径')
parser.add_argument('-dc', '--diffusion_config_path', type=str, default=f"logs/44k/diffusion/config.yaml/false", help='扩散模型配置文件路径')
...
parser.add_argument('-p', '--pad_seconds', type=float, default=1, help='推理音频pad秒数，由于未知原因开头结尾会有异响，pad一小段静音段后就不会出现')
...
if cluster_infer_ratio != 0:
    if args.cluster_model_path == "":
        if args.feature_retrieval:  # 若指定了占比但没有指定模型路径，则按是否使用特征检索分配默认的模型路径
            args.cluster_model_path = f"logs/44k/feature_and_index.pkl"
        else:
            args.cluster_model_path = f"logs/44k/kmeans_10000.pt"
...
def voice_change_model():
    ...
    soundfile.write(out_wav_path, audio, svc_model.target_sample, format="ogg")
    out_wav_path.seek(0)
    return send_file(out_wav_path, download_name="temp.ogg", as_attachment=True)

@app.route("/skip_svc", methods=["POST"])
def skip_change_model():
    request_form = request.form
    wave_file = request.files.get("sample", None)
    input_wav_cache = wave_file.read()
    cache_f = io.BytesIO(input_wav_cache)
    data, samplerate = soundfile.read(cache_f)

    out_wav_path = io.BytesIO()
    soundfile.write(out_wav_path, data, samplerate, format="ogg")
    out_wav_path.seek(0)
    return send_file(out_wav_path, download_name="temp.ogg", as_attachment=True)
